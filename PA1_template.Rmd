...
``` {r prelim, results = "hide", message = FALSE}
library(dplyr)
library(ggplot2)
```
...
From these calculations, we can see that the mean total number of steps taken per day is `r round(mean_steps, 1)`, and the median total number of steps taken per day is `r round(median_steps, 1)`.
...
As we can see, the new histogram does not look appreciably different from the original one, and the new mean and median (`r round(mean_steps2, 1)` and `r round(median_steps2, 1)`, respectively) are also quite similar. In fact, the original mean and the new mean are the same, as expected since we used the mean to impute a missing value.
...
``` {r weekday_weekend_average, message = FALSE}
final <- main2 %>%
